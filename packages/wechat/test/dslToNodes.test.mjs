...
test('image: fit=9:16 竖屏', () => {
  ...
  assert.equal(n.attrs.fitClass, '9-16');  // 冒号→连字符,作 wxml class
});

test('image: fit=fixed 输出 height 供 wxml 设容器高', () => {
  const n = firstChild(dslToNodes('::image url="https://x.jpg" fit=fixed height=400'));
  assert.equal(n.attrs.fit, 'fixed');
  assert.equal(n.attrs.fitClass, 'fixed');
  assert.equal(n.attrs.height, 400);  // wxml 据此 style="height:400rpx"
  assert.equal(n.attrs.mode, 'aspectFill');
});

test('image: 16:9 fitClass 连字符(无冒号)', () => {
  const n = firstChild(dslToNodes('::image url="https://x.jpg" fit=16:9'));
  assert.equal(n.attrs.fitClass, '16-9');
});
